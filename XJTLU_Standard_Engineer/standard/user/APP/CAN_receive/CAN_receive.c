...
//声明电机变量
static motor_measure_t motor_yaw, motor_pit, motor_trigger, motor_chassis[4], motor_arm;
...
//发送机械臂电机控制命令
void CAN_CMD_ARM(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4)
{
...
//返回机械臂电机初始化数据
void get_arm_motor_offset(motor_measure_t* ptr, CanRxMsg *rx_message)
{
    ptr->ecd        = (uint16_t)(rx_message->Data[0] << 8 | rx_message->Data[1]);
...
//返回机械臂电机数据
void get_arm_motor_measure(motor_measure_t* ptr, CanRxMsg *rx_message)
{
		ptr->last_ecd = ptr->ecd;                                                          
    ptr->ecd = (uint16_t)(rx_message->Data[0] << 8 | rx_message->Data[1]);           
    ptr->speed_rpm = (uint16_t)(rx_message->Data[2] << 8 | rx_message->Data[3]);     
...
//返回机械臂电机变量地址，通过指针方式获取原始数据
motor_measure_t *get_Arm_Motor_Measure_Point(void)
{
    return &motor_arm;
}
...
static void CAN_hook(CanRxMsg *rx_message)
{
    ...
    case CAN_3508_M4_ID:
    {
		...
		{
        //处理电机数据宏函数
				motor_arm.msg_cnt++ <= 50 ? get_arm_motor_offset(&motor_arm, rx_message) : get_arm_motor_measure(&motor_arm, rx_message);
        //记录时间
        //DetectHook(ArmMotorTOE);
				break;
		}
