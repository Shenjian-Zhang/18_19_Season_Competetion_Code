...
//单轴陀螺仪数据读取
#define get_black_gyro_data(ptr, rx_message)                                              		 \
    {                                                                                          \
				ptr = (float)(-0.001)*((int32_t)(rx_message->Data[0]<<24)|(int32_t)(rx_message->Data[1]<<16)|(int32_t)(rx_message->Data[2]<<8)|(int32_t)(rx_message->Data[3]));			\
		}
...
//声明单轴陀螺仪变量
static float single_yaw_gyro = 0.0f;
...
//CAN 发送 0x406单轴陀螺仪重置
void CAN_CMD_BLACK_GYRO_RESET(void)
{

    CanTxMsg TxMessage;
    TxMessage.StdId = 0x406;
    TxMessage.IDE = CAN_ID_STD;
    TxMessage.RTR = CAN_RTR_DATA;
    TxMessage.DLC = 0x08;
    TxMessage.Data[0] = 0;
...
//返回单轴陀螺仪变量地址，通过指针方式获取原始数据
const float *get_Single_Yaw_Gyro_Point(void)
{
    return &single_yaw_gyro;
}
...
static void CAN_hook(CanRxMsg *rx_message)
{
    ...
    case CAN_3508_M4_ID:
    {
		...
		case CAN_BLACK_GYRO_ID:
		{
				get_black_gyro_data(single_yaw_gyro, rx_message);
		}
