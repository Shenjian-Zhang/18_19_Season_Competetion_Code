...
static void chassis_spinning_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector);
/**
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    //遥控器设置行为模式
		if(switch_is_down(chassis_move_mode->chassis_RC->rc.s[SPINNING_CHANNEL]) && switch_is_up(chassis_move_mode->chassis_RC->rc.s[MODE_CHANNEL]))
		{
				chassis_behaviour_mode = CHASSIS_SPINNING;
				chassis_spinning_finished = 0;
		}
		else
		{
				if(chassis_spinning_finished == 0) chassis_spinning_finished = 2;
				
				if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[MODE_CHANNEL]) && chassis_spinning_finished == 1)
				{
				...
				else if (switch_is_up(chassis_move_mode->chassis_RC->rc.s[MODE_CHANNEL]) && chassis_spinning_finished == 1)
				{
    ...
    else if (chassis_behaviour_mode == CHASSIS_NO_FOLLOW_YAW)
    {
		...
		{
				chassis_move_mode->chassis_mode = CHASSIS_VECTOR_NO_FOLLOW_YAW_SPINNING;
		}
...
static void chassis_spinning_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    if (vx_set == NULL || vy_set == NULL || chassis_move_rc_to_vector == NULL)
    {
        return;
    }
