...
static void gimbal_spinning_control(fp32 *yaw, fp32 *pitch, Gimbal_Control_t *gimbal_control_set);
...
void gimbal_behaviour_mode_set(Gimbal_Control_t *gimbal_mode_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_ABSOLUTE_ANGLE || gimbal_behaviour == GIMBAL_SPINNING)
    {
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, Gimbal_Control_t *gimbal_control_set)
{
    ...
    rc_deadline_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[PitchChannel], pitch_channel, RC_deadband);
		...
		if(gimbal_control_set->vision_receive_ctrl->difference_angle.yaw == VISION_NO_TARGET_VALUE || gimbal_control_set->vision_receive_ctrl->difference_angle.pitch == VISION_NO_TARGET_VALUE)
		{
		...
		{
				rc_add_yaw = gimbal_control_set->vision_receive_ctrl->difference_angle.yaw * GIMBAL_YAW_VISION_ADD_SEN;
				rc_add_pit = gimbal_control_set->vision_receive_ctrl->difference_angle.pitch * GIMBAL_PITCH_VISION_ADD_SEN;
		}
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		else if (gimbal_behaviour == GIMBAL_SPINNING)
		{
				gimbal_spinning_control(&rc_add_yaw, &rc_add_pit, gimbal_control_set);
		}
...
static void gimbal_behavour_set(Gimbal_Control_t *gimbal_mode_set)
{
    ...
    if (gimbal_behaviour == GIMBAL_INIT)
    {
		...
		//开关控制 云台状态
		if(switch_is_down(gimbal_mode_set->gimbal_rc_ctrl->rc.s[SpinningChannel]) && switch_is_up(gimbal_mode_set->gimbal_rc_ctrl->rc.s[ModeChannel]))
		{
				gimbal_behaviour = GIMBAL_SPINNING;
		}
    else
		{
