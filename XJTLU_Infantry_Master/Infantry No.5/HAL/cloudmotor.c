...
void angle_set_update(struct Angle *angle)
{
	...
	if(angle->back_flag == 0)//缓慢启动回中(采用空间云台)
	{
		...
		{			
			...
			{
				angle->pitch_set = angle->pitch_set < angle->pitch_target ? angle->pitch_set + 1.0f : angle->pitch_set - 1.0f;
				angle->pitch_remote_last = angle->pitch_remote_now = angle->pitch_set;
			}
			if(((int)angle->yaw_set != (int)angle->yaw_target) && ((int)angle->pitch_set == (int)angle->pitch_target))
				angle->yaw_set = angle->yaw_set < angle->yaw_target ? angle->yaw_set + 1.0f : angle->yaw_set - 1.0f;
	...
	{			
		...
		if(rc.mouse_r == 1)
		{
			if(PITCH_SEE == 1)
			{
				angle->yaw_set += rc.L_x / 1500.0f - (rc.mouse_x / 300.0f) + out[VISUAL_YAW_OFFSET];	
				//pitch轴加入限位
		...
		{ 
			//yaw轴不限位
			angle->yaw_set += rc.L_x / 1500.0f - (rc.mouse_x / 300.0f) ;
			//pitch轴加入限位
			angle->pitch_remote_last = angle->pitch_remote_now;
