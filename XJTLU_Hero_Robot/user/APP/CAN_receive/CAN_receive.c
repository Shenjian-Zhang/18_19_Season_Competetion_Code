...
//声明电机变量
motor_measure_t motor_yaw, motor_pit, motor_trigger, motor_chassis[4], motor_shoot_big[3], motor_limit;
...
void CAN_CMD_TRIGGER(int16_t small_trigger)
{
    CanTxMsg TxMessage;
    TxMessage.StdId = CAN_SHOOT_BIG_ALL_ID;
    TxMessage.IDE = CAN_ID_STD;
    TxMessage.RTR = CAN_RTR_DATA;
    TxMessage.DLC = 0x02;
    TxMessage.Data[0] = (small_trigger >> 8);
    TxMessage.Data[1] = small_trigger;
...
//发送大发射机构电机控制命令
void CAN_CMD_SHOOT_BIG(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4)
{
    CanTxMsg TxMessage;
    TxMessage.StdId = CAN_SHOOT_BIG_ALL_ID;
    TxMessage.IDE = CAN_ID_STD;
    ...
    CAN_Transmit(SHOOT_BIG_CAN, &TxMessage);
}
...
//返回大发射机构电机变量地址，通过指针方式获取原始数据
const motor_measure_t *get_Shoot_Big_Motor_Measure_Point(uint8_t i)
{
		return &motor_shoot_big[(i & 0x03)];
}
//返回大发射机构限位电机变量地址，通过指针方式获取原始数据
const motor_measure_t *get_Limit_Motor_Measure_Point(void)
{
		return &motor_limit;
}
...
static void CAN_hook(CanRxMsg *rx_message)
{
    ...
    case CAN_TRIGGER_MOTOR_ID:
    {
		...
		case CAN_SHOOT_BIG_LIMMIT_ID:
		{
        //处理电机数据宏函数
        get_motor_measure(&motor_limit, rx_message);
        //记录时间
//        DetectHook(LimitMotorTOE);
//        break;
		}
    case CAN_3508_M1_ID:
    case CAN_3508_M2_ID:
    ...
    case CAN_3508_M4_ID:
    {
		...
		case CAN_SHOOT_BIG_TRIGGER_ID:
		{
        //处理电机数据宏函数
        get_motor_measure(&motor_shoot_big[2], rx_message);
				break;
		}
